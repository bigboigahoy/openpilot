...
from openpilot.frogpilot.controls.lib.frogpilot_events import Events as FrogPilotEvents, FrogPilotEventName
...
class Controls:
  def __init__(self, CI=None):
    ...
    self.combined_events = Events()
    self.frogpilot_events = FrogPilotEvents()
  ...
  def update_events(self, CS):
    ...
    self.combined_events.events = self.events.events + self.frogpilot_events.events
  ...
  def state_transition(self, CS):
    ...
    if self.state != State.disabled:
      ...
      if self.combined_events.contains(ET.USER_DISABLE):
        self.state = State.disabled
        self.current_alert_types.append(ET.USER_DISABLE)

      elif self.combined_events.contains(ET.IMMEDIATE_DISABLE):
        self.state = State.disabled
        ...
        if self.state == State.enabled:
          if self.combined_events.contains(ET.SOFT_DISABLE):
            self.state = State.softDisabling
            self.soft_disable_timer = int(SOFT_DISABLE_TIME / DT_CTRL)
            self.current_alert_types.append(ET.SOFT_DISABLE)

          elif self.combined_events.contains(ET.OVERRIDE_LATERAL) or self.combined_events.contains(ET.OVERRIDE_LONGITUDINAL):
            self.state = State.overriding
            self.current_alert_types += [ET.OVERRIDE_LATERAL, ET.OVERRIDE_LONGITUDINAL]

        # SOFT DISABLING
        elif self.state == State.softDisabling:
          if not self.combined_events.contains(ET.SOFT_DISABLE):
        ...
        # PRE ENABLING
          ...
          if not self.combined_events.contains(ET.PRE_ENABLE):
            self.state = State.enabled
          else:
        ...
        # OVERRIDING
          ...
          if self.combined_events.contains(ET.SOFT_DISABLE):
            ...
            self.soft_disable_timer = int(SOFT_DISABLE_TIME / DT_CTRL)
            self.current_alert_types.append(ET.SOFT_DISABLE)
          elif not (self.combined_events.contains(ET.OVERRIDE_LATERAL) or self.combined_events.contains(ET.OVERRIDE_LONGITUDINAL)):
            self.state = State.enabled
          else:
            self.current_alert_types += [ET.OVERRIDE_LATERAL, ET.OVERRIDE_LONGITUDINAL]
    ...
    # DISABLED
      ...
      if self.combined_events.contains(ET.ENABLE):
        if self.combined_events.contains(ET.NO_ENTRY):
          self.current_alert_types.append(ET.NO_ENTRY)

        else:
          if self.combined_events.contains(ET.PRE_ENABLE):
  ...
  def state_control(self, CS):
    ...
    CC.longActive = self.enabled and not self.combined_events.contains(ET.OVERRIDE_LONGITUDINAL) and not self.sm['frogpilotCarState'].pauseLongitudinal and self.CP.openpilotLongitudinalControl
  ...
  def publish_logs(self, CS, start_time, CC, lac_log):
    ...
    alerts += self.frogpilot_events.create_alerts(self.current_alert_types, [self.CP, CS, self.sm, self.is_metric, self.soft_disable_timer, self.frogpilot_toggles])
    ...
    controlsState.engageable = not self.combined_events.contains(ET.NO_ENTRY)
    ...
    if frogpilotControlsState.lateralControlState.which() == 'torqueState' and (self.frogpilot_toggles.nnff or self.frogpilot_toggles.nnff_lite):
      frogpilotControlsState.lateralControlState.torqueState.nnLog = self.LaC.nnff.nnLog
