...
AbstractAlert::AbstractAlert(bool hasRebootBtn, QWidget *parent) : QFrame(parent) {
  ...
  disable_check_btn->setStyleSheet(R"(color: #cdd6f4; background-color: #4F4F4F;)");
  ...
  snooze_btn->setStyleSheet(R"(color: #cdd6f4; background-color: #4F4F4F;)");
...
int OffroadAlert::refresh() {
  ...
  if (alerts.empty()) {
    ...
    for (auto &[key, severity] : sorted) {
      ...
      l->setMargin(60);
      l->setWordWrap(true);
      l->setStyleSheet(QString("background-color: %1").arg(severity ? "#E22C2C" : "#313244"));
